---
title: "(Absolute) Area Between Curves of Multiple Discretization Results for One Data Set"
author: "Tiffany Jann"
date: ""
output: 
  html_document:
...
includeSourceDocuments("localarea.Rmd")
```
...
```{r}
pointData = as.data.frame(sapply(pointFiles, getfile))
intervalData = as.data.frame(sapply(intervalFiles, getfile))
#head(pointData)
#head(intervalData)
```
...
```{r}
## Apply diffArea to each point method's data set
scoresPoint = apply(pointData, 2, diffArea, standard=gold)
## Apply diffArea to each interval method's data set
scoresIntvl = apply(intervalData, 2, diffArea, standard=intvlGold)
## Join results for sorting
scores = append(scoresPoint, scoresIntvl)
```
